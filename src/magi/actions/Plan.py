...
class PlanExecutableSolution(ExecutableSolution):
    ...
    def execute(self, env, simulate, validate=False, detector=None):
        ...
        @param validate If True, validate during execution
        @param detector Use this detector during validation
        """
        robot = self.action.get_robot(env)

        with Validate(env, self.precondition, self.postcondition,
                      detector, validate, validating=self):
...
class PlanSolution(Solution):
    def __init__(self, action, path, deterministic,
        ...
        @param deterministic True if this solution was created in a
...
class PlanAction(Action):
    def __init__(self, robot, active_indices, active_manipulator,
                 method, args=None, kwargs=None, planner=None, name=None,
                 checkpoint=False, precondition=None, postcondition=None):
        """
        ...
        @param precondition Precondition of this action
        @param postcondition Postcondition of this action
        """
    ...
    def plan(self, env):
        ...
        poseValidator = RobotPoseValidator(configuration=configuration,
                                           robot=robot,
                                           dof_indices=range(robot.GetDOF()))
        if self.precondition is not None:
            self.precondition = SequenceValidator([self.precondition,
                                                   poseValidator])
        else:
            self.precondition = poseValidator
...
class PlanToTSRAction(PlanAction):
    ...
    def plan(self, env):
        ...
        import prpy
        from hgpc.actions.validators.PoseValidators import ObjectPoseValidator
        from hgpc.actions.validate import SequenceValidator
        with prpy.viz.RenderTSRList(tsr_list, env):
            solution = super(PlanToTSRAction, self).plan(env)

            # Add object pose validator as precondition
            solution.precondition = ObjectPoseValidator(obj.GetName(),
                obj.GetTransform(), name="PlanToTSR-ObjectPoseValidator")
            return solution
...
class PlanEndEffectorStraight(PlanAction):
    def __init__(self, robot, active_indices, active_manipulator, distance,
        ...
        @param planner A specific planner to use - if None, robot.planner is used
        """
        super(PlanEndEffectorStraight, self).__init__(robot,
                                                      active_indices,
                                                      active_manipulator,
