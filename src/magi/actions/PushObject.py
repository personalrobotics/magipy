"""
Define PushObjectAction, PushObjectSolution, and PushObjectExecutableSolution.
"""
...
class PushObjectExecutableSolution(ExecutableSolution):
    ...
    def execute(self, env, simulate):
        """
        Execute the planned pushing trajectory. This action grabs the object
        being pushed for the duration of the push, then releases it. This
        ensures the object will end up in the proper place relative to the hand
        in the OpenRAVE environment.
        ...
        try:
            retval = robot.ExecuteTrajectory(
...
class PushObjectSolution(Solution):
    ...
    def __init__(self, action, path, obj_in_hand_pose, deterministic):
        ...
        @param obj_in_hand_pose: the post of the object relative to the hand
          during the push
    ...
    def save(self, env):
        """
        Return a context manager that saves the current robot and object configurations.
...
class PushObjectAction(Action):
    ...
    def get_push_direction(self, manipulator):
        """
        Get the direction of arm movement.

        @param manipulator: OpenRAVE manipulator that will move
    ...
    def obj_to_hand(self, obj, manipulator, env, push_direction,
        ...
        @param obj: KinBody being pushed
        @param manipulator: OpenRAVE manipulator
        @param env: OpenRAVE environment
        @param push_direction: direction the palm pushes the object
        ...
        with env:
            ...
            stepsize = 0.01
            total_distance = 0.0
            while not env.CheckCollision(robot, obj) and \
                  total_distance <= push_distance:
