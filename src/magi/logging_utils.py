...
def setup_logger():
    ...
    formatter = logging.Formatter('%(asctime)s %(message)s', '%Y%m%d %H:%M:%S')  # date/time plus message
...
def _log_data(path, action_name, header, tag, log_metadata=False):
    ...
    log_data = [header, action_name, 
                path_tags.get(tag, 'unknown')]
    if log_metadata:
        log_data += [ path_tags.get(Tags.PLANNER, 'unknown'),
